...
- **merkaztech-tpack.html** – ניתוח מערכי שיעור/פעילויות לפי מודל TPACK (ראו סעיף 6 למטה).
...
## 6. הפעלת ניתוח TPACK (`merkaztech-tpack.html`)
דף זה מאפשר להעלות מערך שיעור/פעילות כקובץ Word (.docx) או PDF, ומנתח אותו אוטומטית לפי מודל TPACK
...
5. פריסת הפונקציה מתוך שורש הריפו (הקוד נמצא ב-`supabase/functions/tpack-analyze`):
   ...
   ```
6. זהו — הדף `merkaztech-tpack.html` יתחיל לעבוד מיד לאחר הפריסה, בלי שינוי נוסף בקוד.
